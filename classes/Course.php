...
class Course extends Tutor_Base {
	...
	public function save_course_meta( $post_ID, $post ) {
		...
		if ( ! empty( $_POST['_tutor_course_type_presentation'] ) ) {
			$course_type = Input::post( '_tutor_course_type_presentation' );
			update_post_meta( $post_ID, '_tutor_course_type_presentation', $course_type );
		}
...
public function add_course_level_to_settings( $args ) {
		...
		$types       = tutor_utils()->course_type_presentation();
		$course_types = get_post_meta( $course_id, '_tutor_course_type_presentation', true );

		$args['general']['fields']['_tutor_course_type_presentation'] = array(
			'type'        => 'select',
			'label'       => __( 'Type of Cours', 'tutor' ),
			'label_title' => __( 'Enable', 'tutor' ),
			'options'     => $types,
			'value'       => $course_types ? $course_types : 'online',
			'desc'        => __( 'Types:online ,saved ', 'tutor' ),
		);
